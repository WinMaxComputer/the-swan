    ...
    <div class="card ">
      <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <a href="/tour-add" class="btn btn-primary btn-sm">Add New Tour</a>
        </div>
      </div>
      <div class="card-body">
        <form action="{{ route('pages.tour') }}" method="GET" class="mb-4">
          <div class="row">
            <div class="col-md-4">
              <div class="input-group">
                <input type="text" name="search" class="form-control text-white" placeholder="Search tours..." value="{{ request('search') }}">
            ...
            @if(request('search') || request('perPage') || request('lang'))
              <div class="col-md-2">
                <a href="{{ route('pages.tour') }}" class="btn btn-secondary btn-sm mt-1">Clear</a>
              </div>
            @endif
          </div>
        </form>
        ...
        <div class="table-responsive">
          <table class="table table-hover">
            ...
            <tbody>
              @forelse ($tour as $tur)
                <tr>
                  <td class="text-center"><span class="badge badge-info">{{$tur->code}}</span></td>
                  <td><strong>{{$tur->tour_name}}</strong></td>
                  <td><span class="badge badge-default">{{$tur->type}}</span></td>
                  <td title="{{ strip_tags($tur->itinerary) }}">{{ Str::limit(strip_tags($tur->itinerary), 60) }}</td>
                  <td class="text-center"><span class="text-uppercase badge badge-default">{{$tur->lang}}</span></td>
                  <td class="text-center">
                    <a href="{{ route('tour.edit',['tour_code' => $tur->id]) }}" class="btn btn-link btn-icon btn-sm" title="Edit">
              ...
              @empty
                <tr>
                  <td colspan="6" class="text-center text-muted py-5">No tour packages found matching your criteria.</td>
                </tr>
              @endforelse  
            </tbody>
          </table>
